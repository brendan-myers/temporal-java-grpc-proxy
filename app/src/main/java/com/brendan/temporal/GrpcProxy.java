 ...
 */
 ...
 public final class GrpcProxy<ReqT, RespT> implements ServerCallHandler<ReqT, RespT> {
   ...
   public static void main(String[] args) throws IOException, InterruptedException {
    ...
    int target_port = 7233;
    String tls_cert = null;
    String tls_key = null;

    try {
      target_host = System.getenv("TEMPORAL_HOST");
      target_port = Integer.parseInt(System.getenv("TEMPORAL_PORT"));
      tls_cert    = System.getenv("TEMPORAL_TLS_CERT_PATH");
      tls_key    = System.getenv("TEMPORAL_TLS_KEY_PATH");

      if (target_host == null || tls_cert == null || tls_key == null ) {
        throw new Exception();
      }
    } catch (Exception e) {
      System.out.println("Failed to start proxy. Please ensure the following environment variables are set; \n" +
     ...
     ManagedChannel channel = Grpc.newChannelBuilderForAddress(target_host, target_port, tlsBuilder.build())
         .build();
 
     logger.info("Proxy will connect to " + target_host + ":" + target_port);
